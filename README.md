...
### v2.9.0 – Sicherheit, Speicher, Reaktionsfähigkeit, Sync, Seitenleiste, Zusammenführen (26.09.2026)

Umsetzung der Vorschläge aus der v2.8.5-Prüfung (ausser Punkt 3, wie gewünscht: der Anmelde-Token bleibt bewusst in `localStorage`, nicht in `sessionStorage`, damit er den Neustart der App/des Browsers übersteht).

- **Spotify-Zugangsdaten:** Das Spotify-Secret steht nicht mehr fest im Quellcode. Stattdessen gibt es unten in der Seitenleiste einen neuen Knopf **«⚙ Spotify-Zugangsdaten»**: dort selbst eine Spotify-App anlegen und Client-ID/Secret eintragen – diese werden nur lokal auf dem jeweiligen Gerät gespeichert (`localStorage`), nicht in der synchronisierten Bibliothek. **Wichtig:** Ohne eigene Zugangsdaten funktioniert die automatische Spotify-Verknüpfungssuche nicht mehr; das alte, alle Geräte betreffende Secret wurde aus Sicherheitsgründen entfernt und sollte in der Spotify-Entwicklerkonsole zurückgezogen werden. Der YouTube-API-Schlüssel steht weiterhin im Code (das ist bei einer reinen Web-App unvermeidlich) – er sollte in der Google-Cloud-Konsole auf die Website-Adresse (HTTP-Referrer) eingeschränkt werden, falls noch nicht geschehen.
- **Content-Security-Policy:** Zweite Verteidigungslinie gegen eingeschleusten Code (zusätzlich zur Bereinigung von Text-Blättern aus v2.8.5): Ein Sicherheits-Header (CSP) erlaubt im Browser nur noch das eigene, mit der jeweiligen Version fest verknüpfte Programm sowie Google/YouTube/Spotify; ein eingeschleustes `<script>` (z. B. über eine Sicherheitslücke in einer der eingebundenen Bibliotheken) würde vom Browser blockiert.
- **Handy – Speicher beim Zoomen:** Beim starken Hineinzoomen in ein Blatt wird auf dem Handy ein kleineres Speicherbudget für das Bild verwendet als auf dem PC (weiterhin scharf genug), dazu wird nicht mehr benötigter Bildspeicher sofort freigegeben statt erst beim nächsten Aufräumen des Browsers.
- **Backup/Wiederherstellen:** Läuft speicherschonender – die ZIP-Datei wird beim Wiederherstellen nicht mehr komplett in den Arbeitsspeicher geladen, sondern nur die benötigten Teile gelesen; das Erstellen eines Backups kopiert vorhandene Daten nicht mehr unnötig doppelt in den Speicher. Merklich bei grossen Bibliotheken mit vielen/grossen Blättern.
- **Tonart-Umrechnung im Hintergrund:** Läuft jetzt, wo möglich, in einem separaten Hintergrund-Prozess (Web Worker) statt die Bedienoberfläche während der Berechnung zu blockieren. Das Live-Stimmgerät während der Aufnahme misst nur noch rund 25× pro Sekunde statt bis zu 120×/Sekunde – für das Auge weiterhin flüssig, spart aber spürbar Akku/Prozessorlast, gerade auf älteren Handys.
- **Automatischer Abgleich mit Drive:** Läuft alle 2 statt bisher jede Minute und pausiert komplett während des Auftrittsmodus (spart Anfragen/Akku genau dann, wenn es am meisten stört); beim Verlassen des Auftrittsmodus wird sofort nachgeholt, damit Änderungen von anderen Geräten zügig ankommen. Eigene Änderungen werden davon unabhängig wie bisher zeitnah gesichert.
- **Programm-Update im Hintergrund:** pdf.js, Schriften und weitere mitgelieferte Bibliotheken (zusammen ca. 4,7 MB) werden nicht mehr bei jeder neuen Version erneut heruntergeladen, sondern bleiben über Versionen hinweg zwischengespeichert, solange sich ihr Inhalt nicht ändert. Nur die eigentliche App-Datei wird wie gewohnt bei jedem Update neu geladen.
- **Seitenleiste bei vielen Songs:** Die Suche reagiert jetzt entprellt (kein voller Neuaufbau der Liste bei jedem Tastendruck), und die Song-/Blattzuordnung beim Rendern verwendet ein Nachschlagewerk statt die Dateiliste pro Song erneut zu durchsuchen. Bei hunderten Songs deutlich flüssiger, insbesondere beim Tippen in die Suche.
- **Zusammenführen (weitere Randfälle):** Ein Blatt, das auf einem Gerät zwischenzeitlich zu einem anderen Song verschoben wurde, taucht nach dem Abgleich nicht mehr fälschlich zusätzlich im alten Song auf. Werden auf zwei Geräten unabhängig voneinander verschiedene Songs zur selben Setliste hinzugefügt, bevor synchronisiert wurde, gehen diese Ergänzungen nicht mehr verloren (in dem seltenen Fall, dass gleichzeitig auf einem Gerät ein Song aus derselben Setliste entfernt wird, kann er dadurch in Einzelfällen wieder auftauchen – ein deutlich selteneres Problem als der bisherige garantierte Datenverlust).

### v2.8.5 – Prüfung: Sicherheit, Stabilität, Performance (26.09.2026)

Ergebnis einer umfassenden Prüfung des Codes. Gefundene Fehler und Sicherheitslücken sind behoben:

- **Sicherheit**
  - **Text-Blätter:** Eingeschleuster Code in einem Text-Blatt (z. B. aus einem manipulierten Backup oder von jemandem mit Zugriff auf den Drive-Ordner) wurde schon beim Laden ausgeführt, bevor die Bereinigung griff. Jetzt wird im abgeschotteten Dokument geparst.
    - Nur noch eigene Klassen und erlaubte Stile (Schriftgrösse, Zeilenabstand, Schrift, Ausrichtung, fett/kursiv/unterstrichen) werden übernommen, keine Anfragen an fremde Server.
    - Backup und Wiederherstellen bereinigen Text-Blätter ebenfalls.
  - **Teilen:** «Aktualisieren» und «Freigabe beenden» prüfen, ob die gemerkte Ordner-ID wirklich ein eigener «Freigabe – …»-Ordner im App-Ordner ist. Eine manipulierte ID konnte bisher den ganzen App-Ordner in den Papierkorb verschieben.
  - **Externe Links:** werden nur noch als `https://` geöffnet.
  - **Backup-ZIP:** Grössen- und Grenzprüfungen beim Entpacken schützen vor «ZIP-Bomben» und beschädigten Dateien.
  - **Bibliothek:** Beim Zusammenführen werden gefährliche Schlüssel (`__proto__` …) ignoriert.
- **Stabilität und Daten**
  - **Anmerkungen:** Beim schnellen Wechsel zwischen Blättern (erstes Öffnen auf einem Gerät) konnten die Striche eines Blatts in die Anmerkungsdatei eines anderen geschrieben werden. Behoben, ebenso eine Fehlermeldung «Synchronisierung fehlgeschlagen» beim Blattwechsel während des Abgleichs.
  - **Zusammenführen:** Bibliothek wird jetzt auf allen Geräten identisch zusammengeführt.
    - Vorher konnten zwei Geräte bei umsortierten Blättern bzw. gleichzeitigen Links die Bibliothek endlos abwechselnd hochladen, und die neue Reihenfolge ging verloren.
    - Gelöschte Aufnahmen/Links kommen auch dann nicht zurück, wenn ein anderes Gerät sie kurz davor noch geändert hat (z. B. Tonhöhenanalyse).
  - **Offline-Kopien:** Aufnahmen, Backing-Tracks und Referenzmelodien wurden bei jedem Abgleich (jede Minute) aus dem Offline-Speicher gelöscht und jedes Mal neu geladen. Behoben: sie bleiben offline verfügbar.
  - **Löschen:** Schlägt das Löschen in Drive fehl (z. B. offline), bleibt das Blatt beim Song (vorher war es danach unsichtbar), und ein Song wird nur gelöscht, wenn alle Blätter gelöscht werden konnten.
  - **Aufnahme:** Versehentliches Schliessen (Esc/Tippen daneben) während der Aufnahme bzw. mit noch nicht gespeicherter Aufnahme fragt nach. Doppeltes Antippen von «Start» öffnet kein zweites Mikrofon; wird der Dialog während der Mikrofon-Freigabe geschlossen, wird das Mikrofon wieder freigegeben. Gleiches beim Einsingen.
  - **Tonart-Umrechnung:** Beim schnellen Trackwechsel konnte ein Track die Tonspur des vorherigen bekommen. Behoben.
  - **Drive-Anfragen:** Anfragen haben ein Zeitlimit; eine hängende Anfrage blockierte vorher den ganzen Abgleich.
  - **Lokaler Speicher:** Schliesst der Browser die Verbindung zur lokalen Datenbank im Hintergrund (Samsung Internet), wird sie automatisch neu geöffnet, statt Änderungen still nicht mehr zu speichern.
  - **Tonhöhenverlauf:** Sehr lange Aufnahmen zeigen den Verlauf korrekt.
  - Referenzmelodie und Einsingen starten den Ton zuverlässiger.
- **Performance**
  - **Arbeitsspeicher:** Abgespielte Audio-Dateien und Aufnahme-Vorschauen belegen nach dem Stoppen keinen Arbeitsspeicher mehr (vorher bis zum Neuladen, je Track mehrere MB). Der Tonart-Zwischenspeicher hält höchstens 2 Fassungen.
  - **Drive-Upload:** Das Öffnen eines Blatts lädt nicht mehr jedes Mal die ganze Bibliothek nach Drive hoch («zuletzt geöffnet» geht mit dem nächsten Abgleich mit).
  - **Weniger Arbeit im Hintergrund:**
    - Die Blattliste wird nur bei Änderungen lokal gespeichert und neu angezeigt, nicht mehr jede Minute.
    - Die Seitenleiste wird beim Offline-Speichern vieler Blätter gebündelt statt pro Blatt neu gezeichnet.
    - Der A–B-Zeitgeber läuft nur bei aktiver Schleife.
  - **Aufräumen:** Grabsteine (gelöschte Links/Aufnahmen/Aufgaben) werden nach 180 Tagen entfernt, damit `library.json` nicht unbegrenzt wächst. Gespeicherte Tonhöhenanalysen gelöschter Aufnahmen werden mitgelöscht.

### v2.8.4 – Seitenleiste zeigt nur noch Songs (25.09.2026)

- **Fehler behoben:** Beim Anlegen neuer Songs mit Text-Blatt und Scans erschienen die Blätter zusätzlich als eigene Einträge in der Songliste («Text», «Scan mit Notizen», «Scan 2», «Adele_Skyfall_Notizen» …).
  - **Ursache:** Ein zweites Gerät hat die neuen Dateien in Drive gesehen, bevor die Zuordnung zum Song von dort synchronisiert war. Es hat sie dann wie alte Einzelblätter (vor v2.1) automatisch zu eigenen Songs gemacht.
  - **Bereinigung:** Diese Einträge werden beim nächsten Öffnen automatisch entfernt, die Blätter bleiben bei ihrem Song. Hat ein solcher Eintrag eigene Inhalte (Favorit, Tags, Aufnahmen, Aufgaben, von Hand gesetzte Links), bleibt er zur Sicherheit stehen.
  - **Vorbeugung:** Gehört ein neues Blatt schon zu einem Song, wird dafür nie mehr ein eigener Song angelegt. Ein Gerät mit altem Stand kann die gelöschten Einträge nicht zurückbringen.

### v2.8.3 – Mehrseitige Blätter: Blättern unten und durch Weiterscrollen, Modus «Lesen», Notizen verdecken nichts mehr (25.09.2026)

- **Seitenwechsel unten am Blatt:** Bei mehrseitigen PDFs/Scans steht unten in der Mitte gut sichtbar «‹ Seite 1 / 3 **Seite 2 ›**». Am Seitenende wird der Weiter-Knopf kurz hervorgehoben. Die kleinen Pfeile oben bleiben zusätzlich.
- **Blättern durch Weiterscrollen:** Am Ende einer Seite weiter nach unten ziehen oder mit dem Mausrad weiterscrollen → nächste Seite, die oben beginnt. Am Anfang weiter nach oben → vorherige Seite, die am Ende steht. Scrollen innerhalb einer Seite blättert nicht.
- **Neues Werkzeug «📖 Lesen»:** Der Finger (bzw. die Maus) scrollt und blättert nur, statt zu zeichnen.
  - Auf dem Handy/Tablet ohne Stift öffnet jedes Blatt jetzt in diesem Modus. Vorher zeichnete der Finger mit dem Stift-Werkzeug, deshalb liess sich das Blatt kaum nach unten scrollen.
  - Zum Zeichnen einfach Stift, Marker usw. antippen.
- **Notizen im Auftrittsmodus:** Sie bilden jetzt einen eigenen Streifen unten über die ganze Breite, und das Blatt wird darüber eingepasst, statt davon überdeckt zu werden. Beim Ausblenden nutzt das Blatt wieder die ganze Höhe.
- **Notizen in der normalen Ansicht:** Sie sind ein eigener Bereich unter dem Blatt, das Blattende lässt sich bis über den Seitenwechsel-Knopf scrollen.
- Während ein Objekt ausgewählt ist, weicht der Seitenwechsel der Auswahlleiste.

### v2.8.2 – Backing-Tracks auch in den Verknüpfungen (25.09.2026)

- Importierte Backing-Tracks (z. B. von x-minus.pro) erscheinen jetzt auch in «🔗 Verknüpfungen» im Bereich «🎚 x-minus.pro & Backing-Tracks», mit Tonart. Du kannst sie dort direkt im Mini-Player abspielen, pausieren und stoppen. Ein Import aus diesem Bereich erscheint dort sofort.
...
4. Wichtig (seit v2.9.0 empfohlen): Schlüssel einschränken (**Schlüssel bearbeiten**) → **sowohl** *API-Einschränkungen* (nur «YouTube Data API v3» erlauben) **als auch** *Anwendungseinschränkungen* → **HTTP-Verweis-URLs (Websites)** → die eigene Adresse eintragen (z. B. `https://krohnoshub.github.io/*`). Der Schlüssel steht offen in `index.html` (die App läuft ohne eigenen Server) – die Website-Einschränkung verhindert, dass ihn jemand anderes auf einer fremden Seite mitbenutzt.
...
4. **Seit v2.9.0:** Beide Werte **nicht** mehr in `index.html` eintragen, sondern in der App selbst über den Knopf **«⚙ Spotify-Zugangsdaten»** unten in der Seitenleiste – sie werden dann nur auf diesem einen Gerät gespeichert (`localStorage`) und müssen auf jedem Gerät einzeln eingerichtet werden, das die automatische Suche nutzen soll.
...
Nach dem Eintragen des YouTube-Schlüssels: `index.html` (und ggf. `sw.js`, falls neu gebaut) wie gewohnt auf GitHub Pages hochladen. Die Spotify-Zugangsdaten werden direkt in der App eingegeben, keine neue Auslieferung nötig.
...
Automatisierte Tests im Headless-Chromium mit simuliertem Google-Login und simuliertem Drive/YouTube/Spotify/lyrics.ovh/LRCLIB/Mikrofon (**≈755 Prüfungen** in 25 Testdateien; Ausführung über `localhost`, teils mit Handy-Emulation: Viewport, Touch, Pixelverhältnis). Stand v2.9.0: **alle bestanden.** Ein Test im Offline/Sync-Testfall (zweiter Geräte-Kontext nach Ablauf des Tokens) ist gelegentlich zeitkritisch/umgebungsabhängig (nicht reproduzierbar durch eine bestimmte App-Version verursacht – bei Bedarf einfach erneut ausführen).

- **v2.9.0 (49, neu):** Spotify-Einstellungsdialog speichert lokal statt im Quellcode (inkl. Zurücksetzen, Neuladen), CSP-Header vorhanden und wirksam (eigenes Skript läuft, fremdes wird blockiert), Handy-Canvas-Speicherbudget bei Maximalzoom deutlich kleiner als Desktop (weiterhin scharf), `freeCanvas()` gibt Speicher sofort frei, Backup/ZIP-Lesen liest beim Aufbau nur Dateiende+Verzeichnis statt der ganzen Datei (Rundreise weiterhin korrekt), Live-Tonhöhenerkennung gedrosselt (≈25/s, weiterhin flüssig), Tonart-Umrechnung im Web Worker blockiert die Oberfläche nicht, Sync-Zeitgeber auf 2 Minuten verlängert und pausiert im Auftrittsmodus, Nachhol-Abgleich beim Verlassen, Vendor-Cache (pdf.js/Fonts/Icons) bleibt über Versions-Bumps hinweg erhalten statt neu zu laden, App-Cache weiterhin bei jedem Bump aktuell, Seitenleiste bei 400 Songs performant und mit Index statt Doppelsuche gerendert, Suche entprellt (ein Re-Render statt vier bei schnellem Tippen), verschobenes Blatt taucht nach Zusammenführen nicht mehr im alten Song auf, gleichzeitige Setlisten-Ergänzungen zweier Geräte gehen nicht mehr verloren (beide Richtungen geprüft).
- **v2.8.5 (26, neu):** eingeschleustes Skript/Bild/SVG/Handler in einem Text-Blatt wird nicht ausgeführt und entfernt, keine Anfrage an fremde Server, nur erlaubte Klassen/Stile, Backup-HTML und Wiederherstellen bereinigt, `javascript:`-Link nicht geöffnet, manipulierte Freigabe-Ordner-ID löscht nichts (weder App-Ordner noch Blatt), Zusammenführen richtungsunabhängig (Reihenfolge, gleiche Zeit), gelöschte Aufnahme bleibt gelöscht, `__proto__` ignoriert, ZIP-Bombe abgebrochen, Notizen eines anderen Blatts laden verändert den Sync-Zustand nicht, Aufnahme bleibt nach Abgleich offline, fehlgeschlagenes Löschen lässt Blatt beim Song, hängende Drive-Anfrage bricht ab, lokale Datenbank nach Schliessen wieder nutzbar, A–B-Zeitgeber aus, Audio-Speicher freigegeben, Blatt öffnen ohne Sofort-Upload, Aufnahme-Dialog fragt bei Esc während/nach Aufnahme.
- **v2.8.4 (12, neu):** Bibliothek wie deine aktuelle (Song mit Scan + Text-Blatt, dazu Einzel-Einträge «Text»/«Scan …»): Einträge verschwinden aus der Liste, Blätter gehören zum echten Song, Einzel-Song mit eigenem Inhalt und altes Einzelblatt bleiben, frisch auf einem anderen Gerät angelegtes Blatt erzeugt keinen Eintrag, Setliste zeigt auf den echten Song, Sync nach Drive, alter Gerätestand bringt nichts zurück, neuer Song mit Text-Blatt erscheint nur einmal.
- **v2.8.3 (23, neu):** Handy: Blatt öffnet im Modus «Lesen», Seitenwechsel unten sichtbar und bedienbar, Ziehen scrollt statt zu zeichnen, über das Seitenende hinaus ziehen → nächste Seite (oben), über den Anfang → vorherige (am Ende), Notizen als eigener Bereich und Blattende sichtbar, Stift zeichnet wieder, einseitiges Blatt ohne Seitenwechsel; PC: Werkzeug bleibt, Mausrad am Ende/Anfang blättert, Scrollen in der Seite nicht; Auftrittsmodus: Notizen-Streifen über volle Breite, Blatt endet darüber, ohne Notizen wieder volle Höhe, kein Rest nach dem Beenden.
- **v2.8.1/2.8.2 (43, neu):** importierter Backing-Track in den Verknüpfungen (mit Tonart, abspielbar, Stopp, Import dort sofort sichtbar); Standardname (Interpret_Titel, ±Halbtöne, ohne Interpret, unzulässige Zeichen), Transposition aus 9 Dateinamen-Varianten, Import-Dialog (Tonart erkannt, Name folgt der Tonart, Ordner wählen), Drive-Name und Kopie im Ordner mit Standardnamen (nachgebaute Ordner-Schnittstelle), Tonart in der Liste; x-minus-Suche öffnet die richtige Adresse, x-minus-Link speichern/öffnen, nicht im Mini-Player; mit den **echten Verknüpfungen von «Name In Blood»**: «The Next Rap God – Dax» wird beim Laden entfernt und bleibt nach Sync, Zusammenführen mit altem Gerätestand und erneuter automatischer Suche weg, Relevanzregel (Titel muss passen), von Hand entfernte Links/Backing-Tracks kommen über ein anderes Gerät nicht zurück, von Hand wieder hinzufügen geht; Handy: Notizen unter dem Blatt, bis ganz unten scrollbar, Höhe = sichtbarer Bereich, Verschiebung nach dem Tippen zurückgesetzt, Einklappen gemerkt. Angepasste ältere Tests: Backing-Track über den Import-Dialog, Testdaten mit generischen Titeln (würden jetzt zu Recht als unpassend entfernt).
